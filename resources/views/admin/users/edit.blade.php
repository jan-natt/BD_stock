                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="kyc_status" id="kyc_status" class="form-control @error('kyc_status') is-invalid @enderror" required>
                                        <option value="">Select KYC Status</option>
                                        @foreach($kycStatuses as $status)
                                            <option value="{{ $status }}" {{ old('kyc_status', $user->kyc_status) == $status ? 'selected' : '' }}>{{ ucfirst($status) }}</option>
                                        @endforeach
                                    </select>
                                    @error('kyc_status')
                                        <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" name="referred_by" id="referred_by" class="form-control @error('referred_by') is-invalid @enderror" value="{{ old('referred_by', $user->referred_by) }}">
                                    @error('referred_by')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <div class="custom-control custom-checkbox">
                                        <input type="checkbox" name="two_factor_enabled" id="two_factor_enabled" class="custom-control-input" value="1" {{ old('two_factor_enabled', $user->two_factor_enabled) ? 'checked' : '' }}>
                                        <label class="custom-control-label" for="two_factor_enabled">Enable Two-Factor Authentication</label>
                                    </div>
                                </div>
                            </div>
                        </div>
